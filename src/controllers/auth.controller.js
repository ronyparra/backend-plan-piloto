  ...
  login: async (req, res) => {
    try {
      ...
      if (user) {
        ...
        if (match) {
          const permission = formatPermissions(
            await AuthService.getPermissionByIdUser(user.idusuario)
          );
...
export const formatPermissions = (permissionsRaw) => {
  ...
  permissionsRaw.map((form) => {
    ...
    if (!exist) {
      ...
      let perm = {};
      Object.entries(permissionFormat[index].permisos).map(([key, value]) => {
        perm = { ...perm, ...{ [key]: value } };
